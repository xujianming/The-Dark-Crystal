...
void ControllerComponent::onInitialize()
{
    ...
    if(!QObject::connect(InputManager::get(), SIGNAL(sPressed(dt::InputManager::InputCode, const OIS::EventArg&)),
                         this,                SLOT(_onButtonDown(dt::InputManager::InputCode, const OIS::EventArg&))))
    {
    ...
    if(!QObject::connect(InputManager::get(), SIGNAL(sReleased(dt::InputManager::InputCode, const OIS::EventArg&)),
                         this,                SLOT(_onButtonUp(dt::InputManager::InputCode, const OIS::EventArg&))))
    {
    ...
    if(!QObject::connect(InputManager::get(), SIGNAL(sMouseMoved(const OIS::MouseEvent&)),
                         this,                SLOT(_onMouseMove(const OIS::MouseEvent&))))
    {
...
{
    if(!QObject::disconnect(this, SLOT(_onButtonDown(dt::InputManager::InputCode, const OIS::EventArg&))))
    {
            Logger::get().error("Cannot disconnect signal sPressed with " + getName()
                + "'s input handling slot.");
    }

    if(!QObject::disconnect(this, SLOT(_onButtonUp(dt::InputManager::InputCode, const OIS::EventArg&))))
    {
            Logger::get().error("Cannot disconnect signal sReleased with " + getName()
                + "'s input handling slot.");
    }

    if(!QObject::disconnect(this, SLOT(_onMouseMove(const OIS::MouseEvent&))))
    {
